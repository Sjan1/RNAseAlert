---
title: "RNAseAlert assay 07-2: Buffer optimation"
author: "Jan Sklenar"
date: "04/05/2020"
...
```
# Optimizaiton of different buffers. 
The measurement started immediately after Benzoasse was mixed with the substrate on plate.
...
## read in raw data
raw <- read.delim("data/200504_RNAaseAlert_buffer2020-05-04T155905.txt",stringsAsFactors = FALSE)
colnames(raw)
...
p <- df[raw$SampleNo==1|df$SampleNo==2|df$SampleNo==3|df$SampleNo==4|df$SampleNo==5|df$SampleNo==6|df$SampleNo==7|df$SampleNo==8,]
## LOESS regression
p <- ggplot(data = p) +
  aes(x = MeasTimeSec, y=RFU) +
  geom_smooth(aes(color=SampleNo)) +
...
- The buffer from the RNAseAlert was the best.
- This substrate concentration was as in Kellner et al 1.25ul of 2uM stock.
- Storage buffer inhibits Benzoase activity.
- Kinetic is quite fast.
